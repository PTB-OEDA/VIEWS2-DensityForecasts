...
dt1 <- as.data.frame(cmbn_df2019[,c("month_id", "country_id", "ged_sb", "ged_sb_tlag_1")])
...
lastobs <- dt1[dt1$month_id=="478",]
local.last <- local.preds[local.preds$month_id=="478",]
tensor.last <- tensor.preds[tensor.preds$month_id==478,]
glmm.last <- glmm.preds[glmm.preds$month_id==478,]
...
local.P.2020 <- local.NB.2020 <- local.TW.2020 <- vector(mode = "list", length=12)
...
for(i in 1:12)
{
  ...
  local.P.2020[[i]] <- local.P.fc
  local.NB.2020[[i]] <- local.NB.fc
  local.TW.2020[[i]] <- local.TW.fc
}
timestamp()

# Now the same for the other models...

tensor.P.2020 <- tensor.NB.2020 <- tensor.TW.2020 <- vector(mode = "list", length=12)
...
for(i in 1:12)
{
  ...
  tensor.P.2020[[i]] <- tensor.P.fc
  tensor.NB.2020[[i]] <- tensor.NB.fc
  tensor.TW.2020[[i]] <- tensor.TW.fc
}


# glmm -- these are hard to do, since they do not easily admit more time
glmm.P.2020 <- glmm.NB.2020 <- glmm.TW.2020 <- vector(mode = "list", length=12)
...
for(i in 1:12)
{
  ...
  glmm.P.2020[[i]] <- glmm.P.fc
  glmm.NB.2020[[i]] <- glmm.NB.fc
  glmm.TW.2020[[i]] <- glmm.TW.fc
}
timestamp()

save.image("ViEWS2-2020.RData")
