...
dt1 <- as.data.frame(cmbn_df2020[,c("month_id", "country_id", "ged_sb", "ged_sb_tlag_1")])
...
lastobs <- dt1[dt1$month_id=="490",]
local.last <- local.preds[local.preds$month_id=="490",]
tensor.last <- tensor.preds[tensor.preds$month_id==490,]
glmm.last <- glmm.preds[glmm.preds$month_id==490,]
...
local.P.2021 <- local.NB.2021 <- local.TW.2021 <- vector(mode = "list", length=12)
...
for(i in 1:12)
{
  ...
  local.P.2021[[i]] <- local.P.fc
  local.NB.2021[[i]] <- local.NB.fc
  local.TW.2021[[i]] <- local.TW.fc
}
timestamp()

# Now the same for the other models...

tensor.P.2021 <- tensor.NB.2021 <- tensor.TW.2021 <- vector(mode = "list", length=12)
...
for(i in 1:12)
{
  ...
  tensor.P.2021[[i]] <- tensor.P.fc
  tensor.NB.2021[[i]] <- tensor.NB.fc
  tensor.TW.2021[[i]] <- tensor.TW.fc
}


# glmm -- these are hard to do, since they do not easily admit more time
glmm.P.2021 <- glmm.NB.2021 <- glmm.TW.2021 <- vector(mode = "list", length=12)
...
for(i in 1:12)
{
  ...
  glmm.P.2021[[i]] <- glmm.P.fc
  glmm.NB.2021[[i]] <- glmm.NB.fc
  glmm.TW.2021[[i]] <- glmm.TW.fc
}
timestamp()

save.image("ViEWS2-2021.RData")
